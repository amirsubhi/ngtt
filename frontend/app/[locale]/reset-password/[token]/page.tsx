...
export default function ResetPasswordPage({ params }: Props) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      setError(err instanceof ApiError && err.code === 'INVALID_TOKEN' ? t('error') : t('error'));
    } finally {
      setLoading(false);
    }
  }
    ...
    return (
      <div className="flex min-h-screen items-center justify-center px-4">
        <div className="w-full max-w-sm text-center space-y-4">
          <p className="text-base text-green-500">{t('success')}</p>
          <Link href="/login" className="text-sm hover:underline opacity-70">Back to login</Link>
        </div>
      </div>
    );
  }
